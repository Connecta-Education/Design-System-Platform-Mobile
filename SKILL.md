...
- `colors_and_type.css` — drop-in tokens.
  ...
  - Cosmo learning: `--cosmo-bg`, `--cosmo-fg`, `--cosmo-accent`, `--state-learn-*`, `--state-progress-*`.
  - Audience scopes: `--scope-child-*`, `--scope-teacher-*`, `--scope-parent-*`, `--scope-school-*`, `--scope-cosmo-*`.
  - Role badges: `--role-child-*`, `--role-teacher-*`, `--role-parent-*`, `--role-school-*`, `--role-cosmo-*`, `--role-goodtalk-*`.
  - Brand washes (marketing only): `--wash-sunrise/dusk/night/cosmo/meadow/paper/heat/calm`.
...
- **Two AIs, never confused.** GoodTalk = safety (lime/olive coaching + plum alert). Cosmo = Socratic learning (sky/curious-blue, never gives the answer).
- No emoji in Connecta's own voice (incl. Cosmo, GoodTalk). No exclamation marks in product UI.
...
- "You" addresses the reader. "I" is never used (Cosmo has no self). "We" is sparing.
...
- No surveillance vocabulary ("monitor", "track", "scan", "patrol"). No "AI tutor" — Cosmo is a thinking partner.
